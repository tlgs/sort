...
void weak_heap_sink(int32_t arr[], bool r[], size_t j, size_t n) {
  size_t k = 2 * j + 1 - r[j];
  if (k >= n) {
    return;
  }
...
// the weak-heap is a much less known data structure; I could not find
// many (i.e. any) proper non-academic introductions to the subject.
// I recommend the following papers (by Stefan Edelkamp et. al):
